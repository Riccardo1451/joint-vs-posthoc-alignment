...
class UnimodalModelMnist1DCNN(nn.Module):

    def __init__(self, embedding_dim=32, num_classes=10):
        super(UnimodalModelMnist1DCNN, self).__init__()
        self.mnist_encoder = MNIST1DEncoderCNN(input_dim=40, hidden_dim=128, output_dim=embedding_dim)
        self.class_head = nn.Linear(embedding_dim, num_classes)
        self.net = nn.Sequential(self.mnist_encoder, self.class_head)

    def get_embedding(self, x_mnist):
        with torch.no_grad():
            return self.mnist_encoder(x_mnist)

    def forward(self, x_mnist):
        return self.net(x_mnist)


class UnimodalModelDigitsCNN(nn.Module):

    def __init__(self, embedding_dim=32, num_classes=10):
        super(UnimodalModelDigitsCNN, self).__init__()
        self.digits_encoder = DigitsEncoderCNN(input_dim=64, hidden_dim=128, output_dim=embedding_dim)
        self.class_head = nn.Linear(embedding_dim, num_classes)
        self.net = nn.Sequential(self.digits_encoder, self.class_head)

    def get_embedding(self, x_digits):
        with torch.no_grad():
            return self.digits_encoder(x_digits)

    def forward(self, x_digits):
        return self.net(x_digits)