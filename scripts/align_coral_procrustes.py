...
for seed in seeds:
    ...
    # --- Modality gap IN-SAMPLE (alignment set, same data used to fit CORAL + Q) ---
    ...
    # --- Modality gap OUT-OF-SAMPLE (held-out paired test) ---
    _, _, _, mu_norm_oos, cov_trace_oos = compute_modality_gap(embs_digits_paired, embs_mnist1d_paired_final)
    print(f"Modality gap [OOS]        |mu|: {mu_norm_oos:.4f}  tr(Σ): {cov_trace_oos:.4f}")

    # --- Procrustes distance on class centroids (paired test set, out-of-sample) ---
...
print(f"dP (centroidi): {np.mean(cdist):.4f} ± {np.std(cdist):.4f}")
