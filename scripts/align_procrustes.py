...
for seed in seeds:
    ...
    # --- Modality gap IN-SAMPLE (alignment set, same data used to fit Q) ---
    ...
    _, _, _, mu_norm_oos, cov_trace_oos = compute_modality_gap(embs_digits_paired, embs_mnist1d_aligned_paired)
    ...
    centroids_mnist1d = torch.stack([embs_mnist1d_aligned_paired[y_paired == c].mean(dim=0) for c in range(10)])
